...
function downloadTranscript() {
    ...
    let textContent = `====================================================\n`;
    textContent += `TRINITY AI - VOICE CALL TRANSCRIPT\n`;
    textContent += `====================================================\n`;
    textContent += `Phone Number: ${phone}\n`;
    textContent += `Date & Time:  ${dateStr}\n`;
    textContent += `Total Messages: ${currentCallData.conversation.length}\n`;
    textContent += `====================================================\n\n`;
    ...
    currentCallData.conversation.forEach((msg) => {
        ...
        textContent += `${speaker}${time}:\n${msg.text}\n\n`;
    });

    textContent += `====================================================\n`;
    textContent += `Generated by TrinityAI Voice Console\n`;

    const blob = new Blob([textContent], { type: "text/plain;charset=utf-8" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `Transcript_${safePhone}_${dateStr.replace(/[^0-9]/g, "_")}.txt`;
